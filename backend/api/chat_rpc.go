...
func (app *APIApplication) GetMessages(ctx context.Context, req *connect.Request[chatv1.GetMessagesRequest]) (*connect.Response[chatv1.GetMessagesResponse], error) {
	...
	participantRow, err := app.Queries.IsChatParticipant(ctx, database.IsChatParticipantParams{
		ChatRoomIDRef: int(req.Msg.ChatId),
		PersonID:      int(personRow.ID),
	})
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	if participantRow.ParticipantID == 0 {
...
func (app *APIApplication) createChat(ctx context.Context, personOneID int, personTwoID int) (int, error) {
	...
	// create the chat
	chatRow, err := app.Queries.CreateOrGetChatRoom(ctx)
	if err != nil {
		return 0, connect.NewError(connect.CodeInternal, err)
	}

	// add chat participants
	personIDs := []int{personOneID, personTwoID}
	for _, personID := range personIDs {
		err = app.Queries.InsertChatParticipants(ctx, database.InsertChatParticipantsParams{
			ChatRoomIDRef: chatRow.ChatRoomID,
			PersonID:      personID,
		})
		if err != nil {
			slog.Error("can't add chat participants", "error", err, "chat_room_id_ref", chatRow.ChatRoomID, "person_id", personID)
			return 0, connect.NewError(connect.CodeInternal, errors.New("can't add chat participants"))
		}
	}

	return chatRow.ChatRoomID, nil
}
...
func (app *APIApplication) SendMessage(ctx context.Context, req *connect.Request[chatv1.SendMessageRequest]) (*connect.Response[chatv1.SendMessageResponse], error) {
	...
	resp := &chatv1.SendMessageResponse{
		Message: &chatv1.Message{
			Id:        int64(messageRow.MessageID),
			ChatId:    int64(messageRow.ChatRoomIDRef),
