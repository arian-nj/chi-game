...
func NewSessionPlayer(ID int, name string) *SessionPlayer {
...
func NewGameSession(Queries *database.Queries, sessionId int, allSession *AllSession) *GameSession {
...
func (session *GameSession) StartGame() {
	utils.RunBackgroundTask(func() {
		...
		for _, player := range session.Players {
			session.GameState.AddPlayer(player.ID, player.Name, player.Socket)
		}
