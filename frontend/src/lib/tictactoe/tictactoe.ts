import type { BoardSize, BotDifficulty } from './types';
...
export function checkWinner(board: Cell[], size: BoardSize): GameResult | null {
    ...
    for (let row = 0; row <= size - winLength; row++) {
        for (let col = 0; col <= size - winLength; col++) {
            ...
            if (mainWinner) return { winner: mainWinner, line: mainDiagonal };
            ...
            if (antiWinner) return { winner: antiWinner, line: antiDiagonal };
        }
    }
...
function lineWinner(board: Cell[], line: number[]): Player | null {
    ...
    if (first !== 'X' && first !== 'O') return null;
...
export function getBotMove(
    ...
    if (difficulty === 'easy') return pickRandomMove(moves);
    if (difficulty === 'medium') return getMediumMove(board, size, bot, human, moves);
    if (size === 3) return getPerfectMove(board, size, bot, human);
...
function getMediumMove(
    ...
    if (shouldTakeCenter && size === 3 && moves.includes(4)) return 4;
...
function evaluateBoard(board: Cell[], size: BoardSize, bot: Player, human: Player): number {
    ...
    for (const line of collectLines(size, winLength)) {
        ...
        if (botCount > 0) score += 10 ** botCount;
        else if (humanCount > 0) score -= 10 ** humanCount;
    }
